...
const ContactPageSetup = () => {
  ...
  useEffect(() => {
    if (data && data.page) {
      setEditorState(data.page.contactPage.content);
    }
  }, [data]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await postData(`/api/page?scope=contact`, {
        content: editorState,
      });
      response.success
...
ContactPageSetup.requireAuthAdmin = true;
ContactPageSetup.dashboard = true;

export default ContactPageSetup;
